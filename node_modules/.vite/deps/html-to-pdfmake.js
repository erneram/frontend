    ...
    function htmlToPdfMake(htmlText, options) {
            ...
            if (options.defaultStyles.hasOwnProperty(keyStyle) && !options.defaultStyles[keyStyle]) {
              ...
              for (var k in options.defaultStyles[keyStyle]) {
                if (options.defaultStyles[keyStyle][k] === "")
                  ...
                  this.defaultStyles[keyStyle][k] = options.defaultStyles[keyStyle][k];
              }
            }
          } else {
      ...
      this.convertHtml = function(htmlText2) {
        ...
        if (this.removeExtraBlanks)
          htmlText2 = htmlText2.replace(/(<\/?(div|p|h1|h2|h3|h4|h5|h6|ol|ul|li)([^>]+)?>)\s+(<\/?(div|p|h1|h2|h3|h4|h5|h6|ol|ul|li))/gi, "$1$4").replace(/(<\/?(div|p|h1|h2|h3|h4|h5|h6|ol|ul|li)([^>]+)?>)\s+(<\/?(div|p|h1|h2|h3|h4|h5|h6|ol|ul|li))/gi, "$1$4");
            ...
            switch (nodeName) {
                ...
                if (Array.isArray(tbodies)) {
                  ...
                  if (hasRowSpan) {
                    ...
                    if (Array.isArray(header)) {
                      var columnCount = header.some(function(cell2) {
                        return cell2.colSpan > 0;
                      }) ? header.reduce(function(partialCount, cell2) {
                        return partialCount + (cell2.colSpan ? cell2.colSpan : 0);
                      }, 0) : header.length;
              ...
              case "IMG": {
                if (this.imagesByReference) {
                  ...
                  if (index > -1)
                    ret.image = "img_ref_" + imagesByReferenceSuffix + index;
                  else {
              ...
              default: {
                if (nodeName === "DIV" && element.dataset && element.dataset.pdfmakeType === "columns") {
                  ...
                  ret = options.customTag.call(this, { element, parents, ret });
                }
              }
            }
      ...
      this.searchForStack = function(ret) {
        if (Array.isArray(ret.text)) {
          for (var i = 0; i < ret.text.length; i++) {
            if (ret.text[i].stack || ["P", "DIV", "TABLE", "SVG", "UL", "OL", "IMG", "H1", "H2", "H3", "H4", "H5", "H6"].indexOf(ret.text[i].nodeName) > -1)
              return true;
      ...
      this.parseStyle = function(element, ignoreProperties) {
              ...
              switch (key) {
                case "margin": {
                  ...
                  value.forEach(function(val, i2) {
                    value[i2] = _this.convertToUnit(val);
                  });
                ...
                case "background-color": {
                  ...
                  if (res.opacity < 1)
                    ret.push({ key: nodeName === "TD" || nodeName === "TH" ? "fillOpacity" : "opacity", value: res.opacity });
                  break;
                }
                case "text-indent": {
                ...
                default: {
                  if (key === "border" || key.indexOf("border-left") === 0 || key.indexOf("border-top") === 0 || key.indexOf("border-right") === 0 || key.indexOf("border-bottom") === 0) {
                    ...
                    if (nodeName === "IMG" && (key === "width" || key === "height")) {
                      ret.push({ key, value: _this.convertToUnit(value) });
                      break;
                    }
                    ...
                    if (value) {
                      ...
                      if (key === "font-size" && parsedValue === false)
                        break;
        ...
        if (borders.length > 0) {
          ...
          borders.forEach(function(b) {
            ...
            var width2 = properties[0].replace(/(\d*)(\.\d+)?([^\d]+)/g, "$1$2 ").trim();
            var index = -1, i2;
            ...
            if (properties.length > 2) {
              ...
              if (index > -1) {
                ...
                for (i2 = 0; i2 < 4; i2++)
                  borderColor[i2] = _this.parseColor(color2).color;
              }
            }
          });
          for (var i = 0; i < 4; i++) {
      ...
      this.parseColor = function(color) {
        ...
        if (hslRegex.test(color)) {
          ...
          if (decimalColors[0].endsWith("%"))
            decimalValue = decimalColors[0].slice(0, -1) * 360 / 100;
          else
            decimalValue = decimalColors[0] * 1;
