  ...
  <div aria-live="assertive"
    ...
    <div class="flex w-full flex-col items-center space-y-4 sm:items-end">
      ...
      <transition v-for="(notification, index) in notifications" :key="index"
        ...
        leave-from-class="opacity-100" leave-to-class="opacity-0" @mouseover="onMouseOver(notification.id)"
        @mouseleave="onMouseLeave(notification.id)">
        <div v-if="notification.visible"
          class="pointer-events-auto w-full max-w-sm overflow-hidden rounded-lg bg-white shadow-lg ring-1 ring-black ring-opacity-5">
          <div class="p-4">
            <div class="flex items-start">
              <div class="flex my-2 flex-shrink-0">
                <component :is="getIcon(notification.type)" :class="['h-8 w-8', iconColor(notification.type)]"
                  color="gray-400" aria-hidden="true" />
              </div>
              <div class="ml-3 w-0 flex-1 pt-0.5">
                <p class="text-sm font-medium text-gray-900">
                  {{ t('http-errors.' + notification.statusTitle) }}
                </p>
                <p v-if="notification.statusMessage !== undefined" class="mt-1 text-sm text-gray-500">
                  {{ t('http-errors.' + notification.statusMessage) }}
                </p>
              </div>
              <div v-if="notification.type !== 'loading'" class="ml-4 flex flex-shrink-0">
                <button type="button"
